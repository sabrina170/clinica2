...
if (!$resultado) {
    ...
    while ($data = mysqli_fetch_assoc($resultado)) {
        $prod    = json_decode($data["productos_tienda"], true);
        $cate    = json_decode($data["categorias"], true);
        $subcate = json_decode($data["subcategorias"], true);

        foreach ($prod['data'] as $key => $pro) {

            if ($pro['id_producto'] == $producto) {

                $id_prod                = $pro['id_producto'];
                $sku_producto           = $pro['sku_producto'];
                $nombre_producto        = $pro['nombre_producto'];
                $imagen_producto        = $pro['imagenes_producto'];
                $precio_unitario        = $pro['precio_unitario_producto'];
                $precio_oferta          = $pro['precio_oferta_producto'];
                $stock                  = $pro['stock_producto'];
                $desc_corta             = $pro['descripcion_corta'];
                $descripcion_producto   = $pro['descripcion_producto'];
                $adicional_producto     = $pro['adicional_producto'];
                $destacado              = $pro['destacado'];
                $estado                 = $pro['estado_producto'];
                $categoria_producto     = $pro['id_categoria'];
                $subcategoria_producto  = $pro['id_subcategoria'];
                $promocion              = $pro['promocion_producto'];
                $galeria                = json_decode(base64_decode($pro['galeria']));

                $metatitulo             = $pro['metatitulo_producto'];
                $metadescripcion        = $pro['metadescripcion_producto'];
                $keywords               = $pro['keywords_producto'];
                $filtros                = json_decode(base64_decode($pro['filtros']), true);
            }
        }
    }
}
...
<div class="page-wrapper">
    <div class="container-fluid">
        <div class="row m-t-20">
            <div class="col-lg-12">
                <h2 id="ide_prod" data-ide="<?php echo $id_prod ?>"><?php echo stripslashes($nombre_producto); ?>
                    ...
                    <a href="page-productos.php" class="btn btn-success float-right m-r-10"><i class="fal fa-list"></i> Ver productos</a></h2>
